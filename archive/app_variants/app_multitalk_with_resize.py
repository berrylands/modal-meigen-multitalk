#!/usr/bin/env python3
"""
MeiGen-MultiTalk with image resize fix.
"""
...
# Image without flash-attn for now
resize_image = (
...
app = modal.App("multitalk-resize")
...
def generate_with_resize(
    ...
    """
    Generate video with proper image resize.
    """
    import boto3
    import shutil
    import os
    ...
    print("="*60)
    print("MeiGen-MultiTalk with Image Resize")
    print("="*60)
    ...
    # Set environment to prefer xformers over flash attention
    if use_xformers:
        os.environ["XFORMERS_ENABLE"] = "1"
        os.environ["FLASH_ATTN_DISABLE"] = "1"
        print("Using xformers instead of flash attention")
    ...
    try:
        ...
        # Environment variables should handle xformers preference
...
if __name__ == "__main__":
    with app.run():
        print("MeiGen-MultiTalk with Image Resize\n")
        print("Key changes:")
        print("1. Resize image to 896x448 (expected by model)")
        print("2. Use xformers instead of flash attention\n")
        
        result = generate_with_resize.remote(
        ...
        if result.get("success"):
            ...
            print(f"⚡ Using xformers: {result.get('using_xformers', False)}")
