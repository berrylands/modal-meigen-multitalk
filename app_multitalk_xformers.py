#!/usr/bin/env python3
"""
MeiGen-MultiTalk using xformers attention.
This approach modifies the environment to force xformers usage.
"""
...
# Image with xformers but no flash-attn
xformers_image = (
    ...
    .run_commands(
        ...
        # Patch the attention.py file directly during image build
        """cd /content/MultiTalk && python -c "
import re
with open('wan/modules/attention.py', 'r') as f:
    content = f.read()

# Replace the flash attention import check
content = re.sub(r'try:\s*import flash_attn.*?FLASH_ATTN_2_AVAILABLE = True.*?except.*?FLASH_ATTN_2_AVAILABLE = False', 
                 'FLASH_ATTN_2_AVAILABLE = False\\nFLASH_ATTN_1_AVAILABLE = False', 
                 content, flags=re.DOTALL)

# Replace assert with conditional
content = content.replace('assert FLASH_ATTN_2_AVAILABLE', 
                         'if FLASH_ATTN_2_AVAILABLE:\\n        pass  # Would use flash attention\\n    else:\\n        # Fallback to standard attention\\n        import torch.nn.functional as F\\n        # Use PyTorch\\'s built-in attention\\n        return F.scaled_dot_product_attention(q, k, v, dropout_p=dropout_p, is_causal=causal)')

with open('wan/modules/attention.py', 'w') as f:
    f.write(content)
print('Patched attention.py to use PyTorch attention')
"
        """,
    )
)

app = modal.App("multitalk-xformers")
...
def generate_with_xformers(
    ...
    """
    Generate video using xformers attention (pre-patched).
    """
    import boto3
    import shutil
    import os
    ...
    print("="*60)
    print("MeiGen-MultiTalk with XFormers")
    print("="*60)
    ...
    print("Using PyTorch attention (flash-attn patched out)")
    ...
    try:
        ...
        if os.path.exists("output.mp4"):
            ...
            s3_key = f"outputs/multitalk_xformers_{timestamp}_{frame_count}f.mp4"
...
if __name__ == "__main__":
    with app.run():
        print("MeiGen-MultiTalk with XFormers\n")
        print("Attention mechanism patched during image build\n")
        
        result = generate_with_xformers.remote(
        ...
        if result.get("success"):
            ...
            print(f"⚡ Attention: {result.get('attention', 'unknown')}")
