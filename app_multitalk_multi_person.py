#!/usr/bin/env python3
"""
MeiGen-MultiTalk with support for multi-person conversations.
Supports single or multiple audio inputs for multi-speaker video generation.
"""
...
app = modal.App("multitalk-multi-person")
...
# Backward compatible single-person function
@app.function(
    image=multitalk_cuda_image,
    gpu="a100-40gb",
    volumes={
...
def generate_video_cuda(
    ...
    """
    Backward compatible function for single person video generation.
    """
    return generate_multi_person_video.remote(
        prompt=prompt,
        image_key=image_key,
        audio_keys=audio_key,  # Single string
        sample_steps=sample_steps,
        output_prefix="multitalk_single"
    )
